...
import com.local.statetracker.settings.ThemeSettings
...
class AppContainer(context:Application){
    val db=AppDatabase.create(context);val dao=db.dao();val checkIns=CheckInRepository(db);val pressure=PressureRepository(dao);val cycles=CycleRepository(dao);val scheduler=NotificationScheduler(context);val themeSettings=ThemeSettings(context)
}
