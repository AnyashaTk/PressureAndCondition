# Pressure and Condition

Локальное Android-приложение Pressure and Condition для дневных, вечерних и дополнительных check-in, давления, отметок начала цикла, календаря, графиков и CSV-экспорта. Приложение не интерпретирует данные и не даёт медицинских рекомендаций.

## Requirements

- Android Studio с Android SDK 36
- JDK 17 или новее (Gradle использует Java target 17)
- Android 9+ (`minSdk 28`); основной ориентир — Samsung Galaxy S24

## Build

```bash
./gradlew assembleDebug
```

## Tests

```bash
./gradlew test
```

## APK

Debug APK создаётся в `app/build/outputs/apk/debug/app-debug.apk`.

## Architecture

Один Android-модуль: Compose UI → ViewModel → repositories → Room / Health Connect / ML Kit / AlarmManager / CSV exporter. Зависимости создаются вручную в `AppContainer`.

## Data storage

История хранится только локально в Room. DAY и EVENING уникальны в пределах даты; EXTRA не ограничены. Отсутствующее значение сохраняется отдельно от `0` и `false`. Destructive migrations не используются.

## Notifications

Inexact-напоминания планируются около 13:00 и 19:00, восстанавливаются после перезагрузки и смены времени. Deep link содержит исходные дату и слот.

## Health Connect

Опционально импортируются только даты начала `MenstruationPeriodRecord`. При недоступности Health Connect ручные отметки продолжают работать.

## OCR

Фото можно снять системной камерой прямо из редактора или выбрать из галереи. Оно обрабатывается on-device с ML Kit во временном cache-файле, который удаляется после обработки. Распознанные SYS/DIA/пульс всегда показываются для ручной проверки и не сохраняются автоматически.

## Analytics

Ось времени строится из выбранного диапазона и группировки независимо от наличия наблюдений. Пустые дни, недели и месяцы сохраняются и доступны для выбора. Общий переключатель добавляет полупрозрачные отметки начала цикла на все графики без влияния на агрегации.

## Mascots

Today содержит декоративную asset-driven область с локальными персонажами. Анимация работает только пока экран и приложение активны; фоновые сервисы не используются.

## Export

Системный диалог создаёт ZIP с `checkins.csv`, `observations.csv`, `blood_pressure.csv`, `cycle_events.csv`, `metrics.csv` в UTF-8.

## Privacy

Нет backend, рекламы, telemetry или INTERNET permission. Android backup отключён. Удаление приложения удалит локальную БД; для сохранения истории используйте экспорт.

## Appearance

В настройках доступны системная, светлая и тёмная темы. Выбор сохраняется локально в DataStore. Светлая тема использует мягкий розовый фон, тёмная — вишнёвый, а основные действия сохраняют зелёный accent.
